...
changes+=(${INPUT_SUCCESS_MSG})
changes_dev+=("\n*Dev log:*")
changes_ct+=("\n*Tickets log:*")
IFS=$'\n' commits=`git log --oneline --pretty="%h^%s" ${{ github.event.before }}..${{ github.sha }}`
for i in $commits; do
  commit_name=${i#*^}
  commit_hash=${i%^*}
  ...
  if [[ $commit_name == *"CV"* ]]; then
    ...
    request=$(curl -H "Authorization: ${INPUT_CLICKUP_KEY}" -H "Content-Type: application/json" \
